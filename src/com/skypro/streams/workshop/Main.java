...
/**
 * https://github.com/volalex/skypro-workshop-task
...
public class Main {
    ...
    public static long countAllFruitsWithFirstA(String[] fruits) {
        return Arrays.stream(fruits).filter(e -> e.charAt(0) == 'a').count();
    }
    ...
    public static Map<Integer, FruitWithId> collectTwoFruitsWithLongNames(List<FruitWithId> fruits) {
        return fruits.stream().filter(e -> e.getName().length() > 5)
                .limit(2)
    ...
    public static List<String> getAllFruitsFromFilesInLowerCase(List<List<String>> fruitsFromFiles) {
        ...
        return fruitsFromFiles.stream().flatMap(List::stream).map(String::toLowerCase).collect(Collectors.toList());
    }
}